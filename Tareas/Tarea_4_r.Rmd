...
```{r}
ggplot(diamonds, aes(x=cut, y = price, fill=factor(cut)))+
  geom_boxplot()+
  scale_y_log10()
...
ggplot(filtered_price, aes(x=cut,y=depth, fill=factor(clarity)))+
  geom_col()+
  facet_wrap(~ clarity)
...
ggplot(cut_premium, aes(x=clarity,y=maxPrice,fill=factor(clarity)))+
  geom_col()
